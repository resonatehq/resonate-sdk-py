    ...
    with Session() as s:
        while True:
            ...
            if res.status_code == _status_codes.BAD_REQUEST:
                msg = "Invalid Request"
                raise StoreError(msg, "STORE_PAYLOAD")
            if res.status_code == _status_codes.UNAUTHORIZED:
                msg = "Unauthorized request"
                raise StoreError(msg, "STORE_UNAUTHORIZED")
            if res.status_code == _status_codes.FORBIDDEN:
                msg = "Forbidden request"
                raise StoreError(msg, "STORE_FORBIDDEN")
            if res.status_code == _status_codes.NOT_FOUND:
                msg = "Not found"
                raise StoreError(msg, "STORE_NOT_FOUND")
            if res.status_code == _status_codes.CONFLICT:
                msg = "Already exists"
                raise StoreError(msg, "STORE_ALREADY_EXISTS")
        time.sleep(1)
