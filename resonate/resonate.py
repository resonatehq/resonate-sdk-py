...
class Resonate:
    ...
    def schedule[**P](
        self,
        id: str,
        func: Callable[Concatenate[Context, P], Generator[Any, Any, Any] | Any],
        cron: str,
        *args: P.args,
        **kwargs: P.kwargs,
    ) -> Schedule: ...
    ...
    def schedule[**P](
        self,
        id: str,
        func: Callable[Concatenate[Context, P], Generator[Any, Any, Any] | Any] | str,
        cron: str,
        *args: P.args,
        **kwargs: P.kwargs,
    ) -> Schedule:
        """Create a schedule to execute a registered function periodically.

        This method creates a schedule that will automatically invoke the specified
        function according to the provided cron expression. The function will be
        executed durably through Resonate's promise system, ensuring reliability
        and fault tolerance.

        Each scheduled execution creates a unique promise with an ID based on the
        schedule ID and timestamp, preventing duplicate executions and enabling
        automatic recovery from failures.

        Args:
            id (str): Unique identifier for the schedule. Used to update or retrieve
                the schedule later.
            func (Callable[Concatenate[Context, P], Generator[Any, Any, Any] | Any] | str):
                The function to execute on schedule, either as a callable or the
                registered name of the function.
            cron (str): Cron expression defining when to execute the function
                (e.g., "0 9 * * *" for daily at 9am, "*/5 * * * *" for every 5 minutes).
            *args (P.args): Positional arguments to pass to the function on each execution.
            **kwargs (P.kwargs): Keyword arguments to pass to the function on each execution.

        Returns:
            Schedule: The created schedule object, which can be used to monitor or
            modify the schedule.
        ...
        Example:
            ...
            Using options for custom timeout and tags::

                resonate.options(
                    timeout=3600,
                    tags={"env": "prod"}
                ).schedule("hourly_sync", sync_data, "0 * * * *")

            Scheduling a registered function by name::

                resonate.schedule("nightly_cleanup", "cleanup_old_data", "0 2 * * *")
        ...
        if not isinstance(cron, str):
            msg = f"cron must be `str`, got {type(cron).__name__}"
            raise TypeError(msg)
        ...
        # Get function name and version from registry
        name, _, version = self._registry.get(func, self._opts.version)
        opts = self._opts.merge(version=version)

        # Encode function arguments with func name and version (required by bridge)
        encoder = opts.get_encoder()
        headers, data = encoder.encode({"func": name, "args": list(args), "kwargs": kwargs, "version": version})

        # Create promise ID pattern based on schedule ID
        # The actual promise IDs will be generated by the server with timestamps
        promise_id = f"{id}.{{{{.timestamp}}}}.{name}"

        # Get timeout (default to 24 hours if not specified)
        timeout_seconds = opts.timeout if opts.timeout else 86400
        promise_timeout_ms = int(timeout_seconds * 1000)

        # Create promise tags with routing target (same as rpc)
        promise_tags = opts.tags.copy() if opts.tags else {}
        target = opts.target
        promise_tags["resonate:invoke"] = target if utils.is_url(target) else f"poll://any@{target}"

        # Create the schedule using the low-level API
        return self._store.schedules.create(
            id=id,
            cron=cron,
            promise_id=promise_id,
            promise_timeout=promise_timeout_ms,
            description=f"Schedule for {name}",
            promise_headers=headers,
            promise_data=data,
...
class Function[**P, R]:
    ...
    def schedule(self, id: str, cron: str, *args: P.args, **kwargs: P.kwargs) -> Schedule:
        """Create a schedule to execute this registered function periodically.

        This method creates a schedule that will automatically invoke this function
        according to the provided cron expression. The function will be executed
        durably through Resonate's promise system, ensuring reliability and fault
        tolerance across scheduled executions.
        ...
        Example:
            Creating a schedule for a registered function::

                @resonate.register
                def generate_report(ctx: Context, user_id: int):
                    ...

                # Schedule to run daily at 9am
                generate_report.schedule("daily_report", "0 9 * * *", user_id=123)

            Using function options::

                @resonate.register
                def sync_data(ctx: Context):
                    ...
        ...
        return resonate.schedule(id, self._func, cron, *args, **kwargs)
