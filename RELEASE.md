...
`TursoNetwork` (`resonate_turso`, the `resonate-turso` package) runs the Resonate protocol with no
...
account; see `packages/resonate-turso/README.md` for numbers and `experiments/` for the scripts.
...
- **`TursoNetwork`** — the protocol's transition relation over Turso
  ...
  timers, static fleet sharding (`shard` + `owner_of`). Schedules are not
...
- **Boundary uploads** (`push_on="boundary"`, default): a task tenure's
...
- **[critical, ts]** `resonate:origin` headers carrying full dotted ids routed
  requests to phantom databases; headers are now normalized via `origin_of` and
...
- **Cross-node CAS without sharding: answered, not yet shipped.** The sync
  ...
  per acquire), which conflicts with `push_on="boundary"`. **Until then,
  deploy sharded: one writer per workflow.**
- `unblock` does not cross nodes; unsharded fleets pay a slow-path latency
  penalty on cross-node completion (documented in `packages/resonate-turso/README.md`).
