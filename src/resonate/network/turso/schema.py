...
listeners registered against them, its tasks, the armed timeouts, and the outbox
the transition relation appends messages to. Every request is served by exactly
one origin database, which is what makes a request a single-database
transaction.
...
timers without opening every origin database), the message index (so a process
can find messages addressed to it without opening every origin database), and
schedules (which are tenant-scoped by definition -- their promise ids are
templates, so a schedule belongs to no one origin).

The tenant tables are *indexes*, never the authority. The origin database is the
authority for both timers and messages; the tenant rows are mirrored from it
after each commit. A stale index entry is harmless: every timeout transition
re-checks its own due time against the origin database before acting (the spec's
NOT BEFORE rule), and a duplicated message is refused by the version fence. A
*missing* index entry only delays work -- the origin database still holds the
truth, and the next flush restores it.
...
ORIGIN_SCHEMA: tuple[str, ...] = (
    ...
    # Messages the transition relation emitted, durable because they are written
    # in the same transaction as the state change that produced them. ``msg_key``
    # implements the spec's collapse-on-set: an execute message is keyed by task
    # id (a newer dispatch supersedes an older one), an unblock message by
    # promise and address.
    """
